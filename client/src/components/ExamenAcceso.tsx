...
import { FileText, Lock, AlertTriangle, User, CheckCircle2, Clock, ShieldCheck, LayoutGrid, GripVertical, Minimize2, Maximize2, X, ChevronRight, LogOut, Battery, BatteryCharging, Columns, Rows } from "lucide-react";
...
// Sub-componente para las filas
function SecurityRow({ darkMode, text }: { darkMode: boolean; text: string }) {
  return (
    <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-colors ${
      ...
      <CheckCircle2 className={`w-4 h-4 ${darkMode ? "text-emerald-400" : "text-emerald-500"}`} />
